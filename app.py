...
# --- PDF සාදන ආකාරය (නිවැරදි කරන ලද Format එක) ---
def create_pdf(bill):
    pdf = FPDF()
    pdf.add_page()
...
# --- Dashboard Logic ---
    ...
    # --- Admin Dashboard (නැවත ඇතුළත් කරන ලදී) ---
    if st.session_state.role == "Admin":
        ...
        with t1:
            ...
            nr = st.selectbox("Role", ["Admin", "Billing", "Technician"], key="admin_role")
            ...
            for i, user in enumerate(st.session_state.users):
                ...
                if c2.button("Delete", key=f"u_{i}"):
        ...
        with t2:
            ...
            for i, d in enumerate(st.session_state.doctors):
                c1, c2 = st.columns([4,1])
                c1.write(d)
                if c2.button("Delete", key=f"d_{i}"):
        ...
        with t3:
            ...
            for i, t in enumerate(st.session_state.tests):
                ...
                if c2.button("Delete", key=f"t_{i}"):
        ...
        with t4:
            ...
            for i, req in enumerate(st.session_state.cancel_requests):
                st.warning(f"Bill ID: {req['bill_id']} (Requested by: {req['user']})")
    ...
    # --- Billing Dashboard ---
        ...
        with t_saved:
            ...
            # වර්තමාන User ට අදාළ බිල්පත් පෙන්වීම
            ...
            if not my_bills:
                ...
                for b in reversed(my_bills):
                    ...
                    col_b1.write(f"**{b['bill_id']}** | {b['patient']} | {b['date']}")
                    col_b2.download_button("PDF", create_pdf(b), file_name=f"{b['bill_id']}.pdf", key=f"dl_{b['bill_id']}")

        with t_recall:
            st.subheader("Search All Bills")
            ...
            if mode == "Name":
                ...
                sm = st.text_input("Enter Mobile Number")
        ...
        with t_summary:
            st.subheader("Summary & Cancellation")
